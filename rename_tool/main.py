...
def rename_from_root(root: Path) -> None:
    """Walks the provided root directory and renames all video files within the season
     sub-folders to match SXXEXX format."""
    _rename_from_root(root)


def _rename_from_root(root: Path) -> None:
    """
    Function to rename all files within the sub-folders in the provided root path to
    have a SXXEXX prefix
